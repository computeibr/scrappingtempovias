...
- **Fix advisory lock preso (`controller/etl.js`)**: `pg_try_advisory_lock`/`pg_advisory_unlock` (lock de **sessão**) dependiam do unlock cair na mesma conexão do pool Sequelize que adquiriu o lock — sem garantia disso, o lock ficava preso para sempre na conexão original (reaproveitada para outras queries da app), bloqueando todos os ciclos seguintes com `"ETL: outro processo está rodando. Ciclo ignorado."` em ambos os ambientes (local e VPS, banco compartilhado). Substituído por `pg_try_advisory_xact_lock` dentro de um `sequelize.transaction()` — o lock é liberado automaticamente no commit/rollback, sem depender de qual conexão do pool executa cada query. Diagnóstico de incidente: `SELECT * FROM pg_locks WHERE locktype='advisory'` + join em `pg_stat_activity` revela o PID preso; `pg_terminate_backend(pid)` destrava manualmente se necessário.
- **Fix timeout no recycle do browser (`controller/etl.js`)**: `browserInstance.close()`/`puppeteer.launch()` não têm timeout próprio — incidente real (máquina local, Windows) travou indefinidamente durante o recycle do Chromium (a cada `ETL_BROWSER_RECYCLE` ciclos), deixando `isRunning=true` para sempre e, por consequência, a transação do advisory lock (`pg_try_advisory_xact_lock`) aberta e presa no Postgres por 1h+ — bloqueando todos os ciclos seguintes mesmo após o PM2 reiniciar o processo sozinho (`max_memory_restart`), porque a conexão antiga ficava "idle in transaction" no banco. Corrigido com helper `comTimeout()` (`Promise.race` contra `setTimeout`): `close()` tem 15s e, se exceder, força `process().kill('SIGKILL')`; `launch()` tem 30s. Em operação normal (sem travamento) o comportamento não muda — os timeouts só agem quando o Chromium realmente trava.
